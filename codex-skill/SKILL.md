...
Anchor the bilingual caption area to the largest visible playing video. Insert it as an in-flow block immediately below the media container so it matches the video's left edge and width, pushes later page content down, and never covers the picture or playback controls. Recalculate and reattach it on scroll and resize. Fall back to a centered viewport overlay only when no visible video can be found.
